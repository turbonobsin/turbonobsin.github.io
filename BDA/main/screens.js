        ...
        if(item.getType() == WeaponType.Wand || item.getType() == WeaponType.Staff){
            if(!me.smallInvOpen){
                ...
                if(pressOnce("arrowright")){
                    me.spellX++;
                    if(me.spellX >= magicData[me.spellY].length) me.spellX = 0;
                }
            ...
            let col = white;
            if(me.spellY < 0) me.spellY += magicData.length;
            else if(me.spellY >= magicData.length) me.spellY -= magicData.length;
            let width = 10; //60,76
            for(let i = 0; i < me.inv.weapons.length; i++){
                let item = me.inv.weapons[i];
                let n = item.getName();
            ...
            let x = 86+width;
            let y = nob2.bottom-6;
            let sDY = magicData[me.spellY];
            if(me.spellX < 0) me.spellX += sDY.length;
            else if(me.spellX >= sDY.length) me.spellX = sDY.length-1;
            let sDX = sDY[Math.min(sDY.length-1,me.spellX)];
            if(!me.spellMenuOpen){
                let n = sDX.name.toUpperCase();
                nob2.drawRect(x-3,y-4,n.length*4+3,9,black);
                col = sDY[0].col;
                nob2.drawText(n,x+Math.floor(n.length*2),y,black,col);
            }
            else{
                let c = 3;
                for(let i = -1; i <= 1; i++){
                    ...
                    col = sDY[0].col;
                    if(sDY){
                        //sDX = sDY[Math.min(sDY.length-1,me.spellX-(me.spellX>0?1:0))];
                        sDX = sDY[0];
                        let n = sDX.name.toUpperCase();
                        nob2.drawRect(x-3,y-4-yy,n.length*4+3,9,black);
                        if(i == 0 && me.spellX == 0){
                            nob2.drawText(n,x+Math.floor(n.length*2),y-yy,black,col);
                        }
                        else nob2.drawText(n,x+Math.floor(n.length*2),y-yy,col,black);
                        let xx = 0;
                        let c2 = 0;
                        for(let j = 1; j < (sDY.length+(Math.min(me.spellX,sDY.length-1)>0?1:0)-Math.min(me.spellX,sDY.length-1)); j++){
                            c2++;
                            let ind2 = c2+(Math.max(me.spellX-1,0));
                            ...
                            if(spell){
                                ...
                                if(ind2 == me.spellX && ind == me.spellY) nob2.drawText(n,x+Math.floor(n.length*2)+xx,y-yy,black,col);
                                else nob2.drawText(n,x+Math.floor(n.length*2)+xx,y-yy,col,black);
                            }
                        }
                    }
                }
