...
function connect(){
    for(let i = 0; i < objs.length; i++){
        for(let j = i+1; j < objs.length; j++){
            ...
            if(dist < 30){
                let c = [(10-o[2]*o[3])/10*255,(10-o[2]/o[3])/10*255,(nob.ssize-o[0]*o[1])/nob.ssize*255,255];
                nob.drawLine_smart(o[0],o[1],o1[0],o1[1],c,1);
            }
        }
    }
}
...
function update(){
    ...
    for(let i = 0; i < objs.length; i++){
        ...
        //vortex
        if(keys.q){
            o[2] -= dx/dist/2*strength;
            o[3] -= dy/dist/2*strength;
        }

        //rotate
        if(keys.w){
            let ang = Math.atan2(dy,dx)+Math.PI/2;
            let tx = Math.cos(ang)*10 / (250-dist); //100
            let ty = Math.sin(ang)*10 / (250-dist);
            o[2] += tx;
            o[3] += ty;
        }
        ...
        objs.push([
            x,y,tx,ty,black,null,function(){
                for(let i = 0; i < 3; i++) if(Math.random() < 0.2){
                    ...
                    let tx = randScale();
                    let ty = randScale();
                    decor.push([t[0]+tx,t[1]+ty,0.1+tx/1.5,-0.5+ty/2,[60,60,60,100],function(){ //40,40,40,100
                        this[7].lifetime--;
                        this[4][3] -= 2;
    ...
    if(true){
        ...
        if(Math.random() < 0.1) for(let i = 0; i < Math.PI*2; i += Math.PI/4){ //4
            let tx = Math.cos(i+angoff)*2;
            let ty = Math.sin(i+angoff)*2;
            objs.push([nob.centerX,nob.centerY,tx,ty,black,null,function(){
                for(let i = 0; i < 3; i++) if(Math.random() < 0.2){
                    let t = this;
                    let tx = randScale();
                    let ty = randScale();
