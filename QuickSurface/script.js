...
const _chnlogL = [
	...
	[
		"Version: 1.8.0 (6-5-25)",
		"Added toggles in 3D player view to toggle visibility of differents parts",
		"Added toggles in 3D player view to toggle auto copying different parts on the left side of the player to their corresponding right sections"
	]
];
...
function startRecording(fr = 20) {
	capturer = new CCapture({
		format: "gif", workersPath: "", framerate: fr,
		verbose: false,
		motionBlurFrames: 0
	});
	capturer.start();
	...
	init() {
		...
		createDropdown(0, [
			...
			if (i == 0) {
				...
				if (threeCanCont.classList.contains("hide") || threeCanCont.classList.contains("type-slim")) {
					threeCanCont.classList.remove("hide");
					threeCanCont.classList.add("type-normal");
					threeCanCont.classList.remove("type-slim");
					prevCan.classList.add("hide");
					if (!loaded3D) init3D();
					load3D_player("mainPlayer_out");
				}
				else {
					threeCanCont.classList.add("hide");
					threeCanCont.classList.remove("type-normal");
					prevCan.classList.remove("hide");
				}
			}
			else if (i == 3) { //Toggle 3D Player (Slim)
				let threeCanCont = document.getElementById("threeCan-cont");
				if (threeCanCont.classList.contains("hide") || threeCanCont.classList.contains("type-normal")) {
					threeCanCont.classList.remove("hide");
					threeCanCont.classList.add("type-slim");
					threeCanCont.classList.remove("type-normal");
					prevCan.classList.add("hide");
					if (!loaded3D) init3D();
				...
				let threeCanCont = document.getElementById("threeCan-cont");
				if (threeCanCont.classList.contains("hide")) {
					threeCanCont.classList.remove("hide");
					prevCan.classList.add("hide");
					if (!loaded3D) init3D();
					load3D_cube();
...
function update() {
	...
	if (!uniDrag) if (overCanvas || img.curLayer.nob.pixelCount > 0) if (!dragging) if (img.curLayer){
		switch (curTool) {
		case Tools.bezier: {
		...
		// AUTO CLONE at run time
		if(mouseDown.some(v=>!!v)) if(overCanvas){
			runAutoClone(true);
		}
	}
...
function _mouseup(button) {
	...
	// the auto is probably kind of annoying to use bc it adds to the history each time but uhh let's see
	if (overCanvas) {
		runAutoClone(true);
	}
}
function runAutoClone(noHist=false){
	if (autoClone.mirrorLA || autoClone.mirrorLAOuter || autoClone.mirrorLL || autoClone.mirrorLLOuter) {
		let used = [
			autoClone.mirrorLA,
			autoClone.mirrorLAOuter,
			autoClone.mirrorLL,
			autoClone.mirrorLLOuter
		];
		...
		for (let i = 0; i < used.length; i++) {
			...
			let l = project.frames[project.frameI].layers[0];
			if (!l) break;

			/**@type {NobsinCtx} */
			let nob = l.nob;

			let superRegion = regions[i];
			for (const region of allRegions) {
				let x = region.x + superRegion.offx;
				let y = region.y + superRegion.offy;
				let tx = region.tx + superRegion.toffx;
				let ty = region.ty + superRegion.toffy;

				let startI = (x + y * project.w) * 4;
				let targetStartI = (tx + (region.flipX ? region.w : 0) + ty * project.w) * 4;

				for (let y = 0; y < region.h; y++) {
					for (let x = 0; x < region.w; x++) {
						if(region.flipX) targetStartI -= 4;

						nob.setData(targetStartI, [
							nob.buf[startI],
							nob.buf[startI + 1],
							nob.buf[startI + 2],
							nob.buf[startI + 3]
					...
					startI += project.w * 4 - region.w * 4;
					targetStartI += project.w * 4 + (region.flipX ? region.w : -region.w) * 4;
				}
			}
		}

		if(!noHist) _histAdd(HistIds.full, null, "Apply auto clone");
	}
}
