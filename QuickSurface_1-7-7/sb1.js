...
function cloneFrame(li,ti,bare=false){
  ...
  for(let i = 0; i < lf.layers.length; i++){
    let l = createLayer_bare(i,lf.layers[i].name,false);
...
var overCanvas = true;
var project = createNewProject("New Image",16,16);
var img;
...
function toggleLayerVisibility(i,bare=false,val){
  ...
  //to-do add hist support
}
function createLayer_html(i,name){
  let l = img.layers[i];
...
loptions_d.children[2].onclick = function(){
...
function rasterizeLayer(l){
  ...
  for(let i = 0; i < nob.size; i += 4){
    nb[i] = b[i] * l.settings.brightness;
...
function rasterizeFrame(i,skip=1){
    ...
    if(e.full){
      ...
      for(let a = 0; a < e.frames.length; a++){
        ...
        for(let b = 0; b < frame.layers.length; b++){
          ...
          let l = createLayer_bare(b,layer.name,false);
      ...
      switch(e.id){
        ...
        case HistIds.createLayer:
          createLayer_bare(e.data.i,e.data.name);
...
var td_d = document.getElementById("toolData");
var FillMode = {
  flood:0,
  global:1
};
...
function update(){
  ...
  for(let i = 0; i < img.layers.length; i++){
    ...
    /*let nn = new OffscreenCanvas(project.w,project.h);
    nn.getContext("2d").putImageData(new ImageData(l.nob.buf,l.nob.width,l.nob.height),0,0);
    ctx.drawImage(nn,0,0);*/
    if(l.visible) nob.drawImage_basic(lr,0,0);
    //c.getContext("2d").putImageData(new ImageData(l.nob.buf,l.nob.width,l.nob.height),0,0);
  }
  ...
  if(overCanvas || img.curLayer.nob.pixelCount > 0) switch(curTool){
    ...
    case Tools.fill:{
      ...
      if(ts.fillMode == FillMode.flood){
        ...
        //console.log("FILL: ",inds.length,times);
        lastSelLen = img.selCount;
        img.curLayer.nob.initRec();
...
function getCol(b,prev=false,darken=false){
  ...
  if(darken) return tint(c,0.7,0.6);
  return c;
}
...
async function file_exportAs(){
  ...
  const h = await window.showSaveFilePicker({
    ...
    suggestedName:project.name+(project.name.endsWith(".png")?"":".png")
  });
...
async function file_open(){
  ...
  switch(type){
    ...
    case "nbg":{
      fileData.text().then(e=>{
        //project = createNewProject("bob",1,1);
        //project.handle = fileHandle;
        //console.log("Read file:","*pre:"+e);
        frames_d.innerHTML = "";
        layers_d.innerHTML = "";
        ...
        project = createNewProject(fileData.name,parseInt(info[1]),parseInt(info[2]));
        addProject(project);
        project.handle = fileHandle;
        resizeImage(project.w,project.h);
...
initMenus();
/*var menu_openFiles_footer = document.getElementsByClassName("menuFooter")[1];
var menu_saveAsFile_footer = document.getElementsByClassName("menuFooter")[2];
...
menu_saveAsFile_footer.children[1].onclick = function(){
  ...
  if(name.length != 0){
    let res = saveAsFile_bare(name);
    if(res){
      console.log("SAVING AS FILE: ",name);
      document.getElementById("menu_saveAsFile").style.visibility = "hidden";
    }
  }
};*/
...
function _loadCtx(id,l,lkey,f,x,y,temp,lvl=0){
...
function createReference(){
  ...
  imgCan.onmousedown = function(e){
    ...
    for(let j = 0; j < this.parentNode.parentNode.parentNode.children.length; j++){
      let c = this.parentNode.parentNode.parentNode.children[j];
