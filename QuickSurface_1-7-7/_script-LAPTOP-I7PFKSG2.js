...
var overCanvas = true;
var project = createNewProject("New Image",256,256);
var img;
...
class Obj{
  ...
  y = 0;
  type;
  getState(){return this.name+","+this.x+","+this.y}
  loadState(){}
}
function selToNewNob(){
  ...
  calcSelBounds();
  let n1 = registerNob2(null,selBounds[2],selBounds[3]);
...
class TestObj extends Obj{
  ...
  render(/**@type {NobsinCtx}*/n){
    n.drawRect(20,20,50,50,[255,0,0,255]);
  }
  clone(){
...
function addProject(p){
  ...
  createDropdown(0,[
    ...
    if(j1 == 0){
      ...
      if(canCloseProject(p)){
        nameDiv.removeChild(d);
        let ind = 0;
        for(let i = 0; i < allProjects.length; i++){
          if(allProjects[i].name == p.name){ //temp, it won't work if 2 files open have the same name
            ind = i-1;
  ...
  if(project.name == p.name) d.className = "sel";
...
function loadProject(p){
  ...
  for(let i = 0; i < nameDiv.children.length; i++){
    ...
    d.className = "";
    if(allProjects[i].name == project.name) d.className = "sel";
  }
}
