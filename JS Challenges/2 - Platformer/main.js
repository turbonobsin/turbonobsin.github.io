...
class Obj{
    constructor(x,y,w,h,spikes=false){
        let t = this;
        t.x = x;
        t.y = y;
        t.w = w*scale;
    ...
    draw(){
        let t = this;
        if(t.hasSpikes) ctx.fillStyle = "firebrick";
...
class Player extends Obj{
    ...
    async restart(){    
        await wait(20);
        this.x = can.width*0.5-20;
        this.y = can.height*0.2;
        // this._animStart = frames;
        this.disabled = true;
        await wait(100);
        this.disabled = false;
    }
    ...
    draw(){
        ...
        if(t.disabled){
            t.vx = 0;
            t.vy = 0;
            let scale = (frames-t._animStart)/30;
            console.log(scale);
            if(scale > 1) scale = 1;
            let w = t.w*scale;
            let h = t.h*scale;
            ctx.fillRect(Math.floor(t.x)+t.w-w,Math.floor(t.y)+h*0.25,w,h);
            return;
        }
        ctx.fillRect(Math.floor(t.x),Math.floor(t.y),t.w,t.h);
...
//spikes
objs.push(new Obj(0,can.height-30,can.width*0.22,20,true));

//floor
objs.push(new Obj(can.width*0.4,Math.floor(can.height*0.9)-30,can.width*0.09,Math.floor(can.height*0.2)));
objs.push(new Obj(can.width*0.75,Math.floor(can.height*0.8),can.width*0.25,Math.floor(can.height*0.2)));
objs.push(new Obj(can.width*0.6,can.height-50,can.width*0.22,20));
...
document.addEventListener("mousedown",e=>{
    if(e.button == 0){
        if(e.shiftKey){
            ...
            let w = 30;
            let h = 2;
            objs.push(new Obj(mx-w/2,my-h/2,w,h));
        }
    }
    else if(e.button == 2){
        //create grapple
